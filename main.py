...
def main_perdomain(domain):
    ...
    set_hyper_flags(learning_rate=0.09, keep_prob=0.4, momentum=0.85, l2_reg=0.0001, mask_threshold=10)
    set_other_flags(source_domain="Creative", source_year=2004, target_domain=domain[0], target_year=domain[1])
    print('main run DIWS')
    attention_final_ouput, source_sen_len, target_sen_len, numdata_source, numdata_target = DIWS_model.main(
        FLAGS.source_path, FLAGS.target_path, domain[2], domain[3], domain[4], domain[5])

    for threshold in thresholds:
        print('current domain:', domain[0], ' & current threshold: ', threshold)
        set_hyper_flags(learning_rate=0.09, keep_prob=0.4, momentum=0.85, l2_reg=0.0001, mask_threshold=threshold)
        set_other_flags(source_domain="Creative", source_year=2004, target_domain=domain[0], target_year=domain[1])
        ...
        print(mask_target_output[9, :], mask_target_output[1, :].shape)
...
def set_hyper_flags(learning_rate, keep_prob, momentum, l2_reg, mask_threshold):
    """
    ...
    FLAGS.l2_reg = l2_reg
    FLAGS.mask_threshold = mask_threshold
